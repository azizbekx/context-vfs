...
def resolve_conflict(
    ...
    prompt = (
        ...
        "DO NOT FLAG (these are not conflicts — pick auto_resolve, "
        "category 'data_quality' or 'synonym' as appropriate):\n"
        "  - Differences in data size or detail level. A short value and a "
        "longer, more detailed value about the same thing are not in "
        "conflict; pick the more complete one as the winner.\n"
        "  - Silence: an empty / missing / 'N/A' value alongside a populated "
        "one is not a conflict. The populated value wins.\n"
        "  - Temporal updates: a later observation reporting a normal "
        "lifecycle change (e.g. status 'pending' -> 'shipped') is not a "
        "contradiction; pick the newer value (category 'temporal_change').\n"
        "  - Different aspects of the same entity: values that describe "
        "different facets (one is a price, the other a delivery date) are "
        "not in conflict. If the predicate isn't actually shared, prefer "
        "'data_quality'.\n"
        "  - Approximate vs precise wording: 'around $400' vs '$420.50' or "
        "'next week' vs 'March 5' are not contradictions unless genuinely "
        "incompatible (category 'synonym').\n\n"
